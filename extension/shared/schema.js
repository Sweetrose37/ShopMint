...
export function normalizePackage(raw){
  ...
  return {...raw,tags,price:Number(raw.price),quantity:Number(raw.quantity),schemaVersion:String(raw.schemaVersion),creationDetails:raw.creationDetails&&typeof raw.creationDetails==='object'?raw.creationDetails:null,mockups:Array.isArray(raw.mockups)?raw.mockups:[],sourceFiles:Array.isArray(raw.sourceFiles)?raw.sourceFiles:[],imageChecklist:Array.isArray(raw.imageChecklist)?raw.imageChecklist:[],deliveryChecklist:Array.isArray(raw.deliveryChecklist)?raw.deliveryChecklist:[]};
}
