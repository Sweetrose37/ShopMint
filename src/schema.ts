...
import {normalizeEtsyTags} from './etsyTags';
...
export function listingReadiness(p:Project){
  ...
  const checks=[
    ...
    {label:'13 Etsy-valid tags',ok:normalizedTags.length===13&&normalizedTags.every((tag,index)=>tag===p.listing.tags[index]?.trim()),critical:false},
