...
async function render(kind:GalleryKind,o:Options,images:HTMLImageElement[],theme:Theme){const c=canvas(),ctx=c.getContext('2d')!;background(ctx,theme);const count=images.length;const headline=marketplaceHeadline(o.productName,count);
  if(kind==='hero'){
    ...
    ctx.fillStyle=theme.accent;round(ctx,70,965,300,112,56);ctx.fill();label(ctx,`${count} DESIGNS`,220,1008,theme.ink,34,'center',800);label(ctx,'DTF PNG BUNDLE  •  DIGITAL DOWNLOAD',70,1120,'#fff',21);
  } else if(kind==='collage'){
    label(ctx,'THE FULL COLLECTION',60,70,theme.accent,22);label(ctx,`${count} DESIGNS. ONE COHESIVE BUNDLE.`,60,116,'#fff',36);
    ...
    ctx.fillStyle=theme.paper;round(ctx,50,50,1100,1100,36);ctx.fill();label(ctx,'WHAT YOU GET',95,115,theme.ink,46);label(ctx,`${count} ORIGINAL DESIGNS`,95,170,theme.accent2,24);
    const shown=images.slice(0,5);shown.forEach((im,i)=>card(ctx,im,90+i*202,240+(i%2)*55,190,290,(i-2)*.025,theme));
    const bullets=[`${count} high-resolution PNG files`,'Transparent backgrounds','Ready for DTF production','Instant digital delivery'];bullets.forEach((text,i)=>{ctx.fillStyle=theme.accent;ctx.beginPath();ctx.arc(120,855+i*62,13,0,Math.PI*2);ctx.fill();label(ctx,'✓',120,855+i*62,theme.ink,15,'center');label(ctx,text,155,855+i*62,theme.ink,25)});
    ...
    const first=o.files[0];label(ctx,`${count} FILES  •  ${first?.width&&first?.height?`${first.width} × ${first.height} PX  •  `:''}ORIGINAL ART PRESERVED`,65,845,'#fff',22);ctx.strokeStyle=theme.accent2;ctx.lineWidth=3;ctx.strokeRect(65,900,1070,205);multiline(ctx,'No physical product will be shipped. Colors may vary by monitor and printing setup.',95,955,1000,38,'#fff',24,500);
  } else {
